...
-- CONSULTAS
-- Registros únicos en la tabla Ordenes
SELECT 
  COUNT (OrdenID) AS RegistrosUnicosOrdenes
  FROM Ordenes;

-- Empleados por Departamento
SELECT 
  Departamento,
  COUNT (EmpleadoID) AS Empleados
  FROM Empleados
...
-- Clientes importado a la tabla Clientes
SELECT
  COUNT (ClienteID) AS Clientes
 FROM
   Clientes;

/* Análisis de desempeño de sucursales: Cuáles son las sucursales con un promedio de Facturación/Ingresos 
superior a 1000.00 y que minimizan sus costos en base al promedio de kilómetros recorridos de todas de sus entregas gestionadas? 
Para un mejor relevamiento, ordene el listado por el Promedio Km Recorridos. */
SELECT
  s.Nombre AS NombreSucursal,
  AVG (o.TotalCompra) AS Ingresos,
  AVG (KilometrosRecorrer) AS KmRecorridos
FROM Ordenes AS o, Sucursales AS s
WHERE s.SucursalID = o.SucursalID
GROUP BY s.Nombre
